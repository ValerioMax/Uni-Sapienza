...
int write_to_pipe(int fd, const void *data, size_t data_len) {
    ...
    if (ret != data_len) handle_error("non tutti byte scritti\n");
...
void reader(int reader_id, sem_t* read_mutex) {
     ...
     **/
    if (close(pipefd[1]) == -1) handle_error("errore chisura descrittore scrittura\n");
     ...
     * - gestire eventuali errori
     **/
    if (close(pipefd[0]) == -1) handle_error("errore chisura descrittore lettura\n");
...
int main(int argc, char* argv[]) {
     ...
     * - gestire eventuali errori
     **/
    if (pipe(pipefd) == -1) handle_error("errore creazione pipe\n");
