...
void build_http_response(const char *file_name, 
    ...
    if (file_fd < 0) //perror("Errore apertura");
    {
    ...
    char *header = (char *) malloc(sizeof(BUFFER_SIZE));
...
void *handle_client(void *arg) {
    ...
    if (bytes_received > 0) {
         ...
         *************************************************************************/
            ...
            {
                ret = send(client_fd, response + bytes_sent, response_len - bytes_sent, 0);
                if (ret == -1 && errno == EINTR) continue;
                if (ret == -1) perror("Errore send");
                bytes_sent += ret;
            }
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (ret == -1) perror("Errore creazione thread");
        /*************************************************************************
