...
void connection_handler(int socket_desc, struct sockaddr_in* client_addr) {
    ...
    **/
    ...
    if (!inet_ntop(client_addr->sin_family, &(client_addr->sin_addr), client_ip, INET_ADDRSTRLEN))
        handle_error("errore conversione");
...
void mprocServer(int server_desc) {
    ...
    while (1) {
        ...
        else if (pid == 0)
        {
            ...
            if (DEBUG) printf("Connection has been handled");
...
void *thread_connection_handler(void *arg) {
    ...
    **/
    connection_handler(((handler_args_t *) arg)->socket_desc, ((handler_args_t *) arg)->client_addr);
    free(((handler_args_t *) arg)->client_addr);
    free(arg);
    pthread_exit(NULL);    
}
...
void mthreadServer(int server_desc) {
    ...
    while (1) {
        ...
        if (ret == -1) handle_error("errore creazione thread");

        if (DEBUG) fprintf(stderr, "Creato nuovo thread\n");

        ret = pthread_detach(thread);
        if (ret == -1) handle_error("errore detach thread");
