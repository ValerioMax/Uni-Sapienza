...
int main()
{
  ...
  memset(&msg, 0, sizeof(struct Message));
    ...
    if (socket_desc < 0) perror("Errore creazione socket");

    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = INADDR_ANY;
    addr.sin_port = htons(port);

    int rc = bind(socket_desc, (struct sockaddr *) &addr, addr_len);
    if (rc == -1) perror("Errore bind");
  ...
  while (1) {
     ...
     **/
    nbytes = recvfrom(socket_desc, buffer, sizeof(buffer), 0, (struct sockaddr *) &client_addr, &addr_len);
    if (nbytes == -1) perror("Errore recvfrom");
   ...
   /** END WITH server in ascolto **/
     ...
     * 7. inviare la risposta DNS 
          ...
          - inserendo la richiesta in nbytes.
     **/
    rc = sendto(socket_desc, buffer, buflen, 0, (struct sockaddr *) &client_addr, addr_len);
    if (rc == -1) perror("Errore sendto");
