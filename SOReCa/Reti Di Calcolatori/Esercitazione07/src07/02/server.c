...
void* connection_handler(int socket_desc) {
    ...
    while (1) {
        ...
        recv_bytes = 0;
        do {
            ret = recvfrom(socket_desc, buf + recv_bytes, 1, 0, (struct sockaddr_in *) &client_addr, &len);
        ...
        while ( bytes_sent < recv_bytes) {
            ret = sendto(socket_desc, buf + bytes_sent, recv_bytes - bytes_sent, 0, (struct sockaddr *) &client_addr, sizeof(struct sockaddr_in));
            if (ret == -1 && errno == EINTR) continue;
