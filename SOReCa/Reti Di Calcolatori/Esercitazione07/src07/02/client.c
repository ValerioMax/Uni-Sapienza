...
int main(int argc, char* argv[]) {
    ...
    while (1) {
	    ...
	    recv_bytes = 0;
    	do {
            ret = recvfrom(socket_desc, buf + recv_bytes, buf_len - recv_bytes, 0, (struct sockaddr *) &server_addr, &len); // potevo mettere NULL, NULL alla fine per non salvare il mittente
            if (ret == -1 && errno == EINTR) continue;
