...
void* connection_handler(int socket_desc) {
    ...
    while (1) {
         ...
         */
        // Faccio do while così non devo ripulire preliminarmente il buffer
        // per il controllo sul '\n'
        recv_bytes = 0;
        do
        {
            ret = recv(socket_desc, buf + recv_bytes, 1, 0); // leggo un byte alla volta per non aspettare all'infinito (credo: es: leggo 4 byte alla volta, ma il messaggio finisce al secondo con '\n' --> la recv aspetterebbe all'infinito per leggere il terzo e quarto byte)
                                                             // forse mi sbaglio: se recv riesce a leggere almeno un byte anche se è comandata di leggerne di più non si blocca (?)
            if (ret == -1 && errno == EINTR) continue; // errore perché la sys call recv è stata interrotta
            if (ret == -1) handle_error("errore recv"); // altro errore
            if (ret == 0) break; // altro end point ha chiuso la connessione
            recv_bytes += ret;
        } 
    ...
    if (ret < 0) handle_error("errore close");
...
int main(int argc, char* argv[]) {
    ...
    if (socket_desc < 0) handle_error("errore creazione socket");
    ...
    if (ret == -1) handle_error("errore bind");
    ...
    if (ret == -1) handle_error("errore listen");
    ...
    while (1) {
         ...
         */
        client_desc = accept(socket_desc, (struct sockaddr *) &server_addr, (socklen_t *) &sockaddr_len);
